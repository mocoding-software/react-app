...
export class Html extends React.Component<HtmlProps> {
  public render(): JSX.Element {
    ...
    const topInlineScripts = (inlineScripts || []).filter((_) => _.position === "top").map((_) => _.script);

    const bottomInlineScripts = (inlineScripts || []).filter((_) => _.position === "bottom").map((_) => _.script);
